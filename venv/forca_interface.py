...
#interfaces
#classe que representa a primeira interface para escolha se deseja ser coordenador ou jogador.
class Inicio(npyscreen.Form):
    ...
    def activate(self):
        self.edit()
        if self.tipo_de_par.value[0]:
            self.parentApp.setNextForm("PROCURANDO_JOGADORES")

#tela que sinaliza a procura de jogadores:
class ProcurandoJogadores(npyscreen.Form):
    def create(self):
        self.jogo = Jogo("helloWorld","novaDica")
...
#aplicativo forca, o ponto de partida do aplicativo, gerencia as interfaces e rotas:
